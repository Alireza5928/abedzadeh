  ...
  if group_lang then
   return reply_msg(msg.id,'<code>》ربات اد نشده بود:\n》به گروه:</code> '..msg.to.title..'\n', ok_cb, false)
    ...
    return reply_msg(msg.id,'<i>》bot Not Added\n》In Group:</i> '..msg.to.title..'\n', ok_cb, false)
		...
		if matches[1]:lower() == 'kick' and is_momod(msg) or matches[1]:lower() == 'ban' and is_momod(msg) or matches[1] == "بن" and is_momod(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
				...
				get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1]:lower() == 'ban' and matches[2] and string.match(matches[2], '^%d+$') then
			...
			elseif matches[1]:lower() == "ban" and matches[2] and not string.match(matches[2], '^%d+$') then
			--[[local cbres_extra = {
					channelid = msg.to.id,
					get_cmd = 'channel_block',
					sender = msg.from.id
